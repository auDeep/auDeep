...
# The ComParE 2018 Self Assessed Affect audio files differ in length. By setting the --fixed-length option, we make
# sure that all audio files are exactly 8 seconds long. This is achieved by cutting or zero-padding audio files as
# required.
fixed_length="8"
...
parser="audeep.backend.parsers.compare18_sas.Compare18SASParser"
...
for clip_below_value in ${clip_below_values}; do
    spectrogram_file="${spectrogram_base}/${taskName}-${window_width}-${window_overlap}-${mel_bands}${clip_below_value}-1-90.nc"
...
for clip_below_value in ${clip_below_values}; do
    ...
    run_name="${output_base}/${taskName}-${window_width}-${window_overlap}-${mel_bands}${clip_below_value}-1-90/t-${num_layers}x${num_units}-${bidirectional_encoder_key}-${bidirectional_decoder_key}"
...
if [ ! -f ${fused_file} ]; then
    echo audeep${verbose_option} fuse --input ${output_base}/${taskName}-${window_width}-${window_overlap}-${mel_bands}*-1-90/*/representations.nc --output ${fused_file}
    echo
    audeep${verbose_option} fuse --input ${output_base}/${taskName}-${window_width}-${window_overlap}-${mel_bands}*-1-90/*/representations.nc --output ${fused_file}
fi
...
export_basename="ComParE2018_SelfAssessedAffect.auDeep"
