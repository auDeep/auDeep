...
# We use 320 ms Hann windows to compute spectrograms
window_width="0.32"

# We use 160 ms overlap between windows to compute spectrograms
window_overlap="0.16"

# Mel-scale spectrograms with 128 frequency bands are extracted
mel_bands="128"

# The ComParE 2018 Heartbeat audio files differ in length. By setting the --fixed-length option, we make sure that all
# audio files are exactly 30 seconds long. This is achieved by cutting or zero-padding audio files as required.
fixed_length="30"
...
parser="audeep.backend.parsers.compare18_heartbeat.Compare18HeartbeatParser"
...
export_basename="ComParE2018_Heartbeat.auDeep"
