#!/bin/bash
# Copyright (C) 2019 Shahin Amiriparian, Michael Freitag, Maurice Gerczuk, Sergey Pugachevskiy, Björn Schuller
#
...
# The ComParE 2019 ContinuousSleepiness (CS) audio files differ in length. By setting the --fixed-length option, we make sure that all
# audio files are exactly 4 seconds long. This is achieved by cutting or zero-padding audio files as required.
fixed_length="4"
...
parser="audeep.backend.parsers.compare19_cs.Compare19CSParser"
...
export_basename="ComParE2019_ContinuousSleepiness.auDeep"
